...
export const PostMansager = (props: Props) => {
  ...
  const handleNavigatePost = () => {
    navigate(`/comment/${props.idPostagem}`); 
  };

  const incrementaVotosPositivos = (e: React.MouseEvent) => {
    e.stopPropagation(); // impede que o clique navegue
    setVotosPositivos(votosPositivos + 1);
  };

  const incrementaVotosNegativos = (e: React.MouseEvent) => {
    e.stopPropagation();
    setVotosNegativos(votosNegativos + 1);
  };

  const handleNavigateComentarios = (e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  return (
    <div
      className="flex flex-col w-[600px] border-solid border-black gap-[10px] font-poppins justify-center cursor-pointer"
        ...
        <div className="flex flex-row gap-[50px] p-[10px]">
          <div className="flex flex-row gap-[10px] bg-gray-200 p-[10px] rounded-[20px] w-[150px] justify-around">
            <div
              ...
              onClick={incrementaVotosPositivos}
            >
              <img
                src={RowTopIcon}
                alt="Row Top icon"
            ...
            <div
              className="flex flex-row gap-[10px] items-center"
              onClick={incrementaVotosNegativos}
            >
              <img
                src={RowDownIcon}
                alt="Row Down icon"
          ...
          <div
            className="flex flex-row gap-[10px] bg-gray-200 pl-[20px] pr-[20px] rounded-[20px] items-center"
            onClick={handleNavigateComentarios}
          >
            <img
              src={CommentIcon}
              alt="Comment icon"
